...
- Use the committed lockfile with `npm ci`, `pnpm install --frozen-lockfile`, or equivalent.
...
- GitHub Actions permissions should default to `contents: read` or `{}`; grant write permissions only per job.
...
- Require review for `.github/workflows/`, `.github/actions/`, package manifests, lockfiles, `.npmrc`, and publish config.
...
If a compromised package install ran: assume local/user secrets are burned, revoke npm/GitHub/cloud tokens, rotate deploy secrets, compare published tarballs against source, and rebuild the machine instead of cute cleanup theater.
